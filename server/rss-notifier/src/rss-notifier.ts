...
import { pool, selectAllRss } from './db-access.js';
...
const findNewFeeds = async (rssObj: FeedObj, now: number) : Promise<FeedDetail[]> => {
    try{
        ...
        const filteredFeeds = feeds.filter(item => {
            ...
            const timeDiff = (now - pubDate) / (1000 * 60 * 60 * 24);
            return timeDiff >= 0 && timeDiff < 1;
        });

        const detailedFeeds = await Promise.all(filteredFeeds.map(async (feed) => {
        ...
        logger.warn(`[${rssObj.rss_url}] 에서 데이터 조회 중 오류 발생으로 인한 스킵 처리. 오류 내용 : ${err}`);
        return [];
    }
}
...
const performTask = async () => {
    ...
    let idx = 0;
    const newFeeds = await Promise.all(rssObjects.map(async (rssObj) => {
        idx += 1;
        logger.info(`[${idx}번째 rss [${rssObj.rss_url}] 에서 데이터 조회하는 중...`);
