...
export const adminRss = async (): Promise<AdminRss> => {
  const response = await axiosInstance.get("/api/rss");
  return response.data;
};

export const adminAcceptRss = async (data: AdminRequest): Promise<AdminResponse> => {
  const response = await axiosInstance.post(`/api/rss/accept/${data.id}`);
  return response.data;
};

export const adminRejectRss = async (data: AdminRequest): Promise<AdminResponse> => {
